...
const Signin = () => {
  ...
  const onSignUpHandle = async () => {
    try {
      const res = await axios.post("http://localhost:3000/api/users/sign-in", user)
      ...
      <div className='w-1/2 bg-slate-200 p-12 m-auto  pt-12'>
        {
          loading ? <p className='text-white'>Loading</p> :
            <div className=''>
              <h2 className='text-4xl py-4 text-center uppercase'>Sign In </h2>
              <div className='flex flex-col gap-2 '>
                ...
                <Link href={"/sign-up"} className='text-center'> or Create account </Link>
              </div>
            </div>
        }
      </div>
